...
contract ERC20Token {
    ...
    modifier  notLocked(address user) {
        require(block.timestamp >= _lockUntil[user],"Token are time-locked");
        _;
    }
    ...
    constructor(string memory _name, string memory _symbol,uint256 initialSupply, uint256 maxCap){
        require(maxCap >= initialSupply, "cap should be >= initial supply");
        name = _name;
        symbol = _symbol;
        owner = msg.sender;
    ...
    function lockTokens(address user, uint256 unlockTime) external onlyOwner{
        uint256  currentunlockTime = block.timestamp + unlockTime;
        require(currentunlockTime > block.timestamp, "unlock time must be in future");
        _lockUntil[user] = currentunlockTime;
        emit Locked(user, currentunlockTime);
    }
    ...
    function burn( uint256 amount)external notLocked(msg.sender) whenNotPaused{
    ...
    /// @param amount Amount to burn (in smallest unit)
    function burnFrom(address account, uint256 amount)external notLocked(msg.sender) whenNotPaused {
    ...
    function approve(address spender, uint256 amount) external whenNotPaused notLocked(msg.sender) returns(bool){
        _approve(msg.sender, spender,  amount);
        return true;
    }

    /// @notice Transfer tokens using allowance
    function transferFrom(address from, address to, uint256 amount) external whenNotPaused notLocked(from) returns(bool){
